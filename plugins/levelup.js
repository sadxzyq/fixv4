...
*${before}* ➔ *${user.level}* [ *${user.role}* ]`.trim()
        ...
        const img = await levelup(teks, user.level)
            conn.send3ButtonImg(m.chat, await(await fetch(hh)).buffer(), `Cᴏɴɢʀᴀᴛᴜʟᴀᴛɪᴏɴ, Aɴᴅᴀ Tᴇʟᴀʜ Nᴀɪᴋ Lᴇᴠᴇʟ!!\n⚙ Lᴇᴠᴇʟ : ${user.level}\n⚙ Rᴏʟᴇ : ${user.role}`, botdate, 'Menu', '.menu', 'Owner', '.owner', 'TestSpeed', '.tes', m, { contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: 'sig',
    mediaType: 1, 
    description: sig,
    ...
    sourceUrl: sig
       ...
       /*     conn.sendButton(m.chat, str, botdate, [['INVENTORY', '.inv']], m)*/
            conn.send3ButtonImg(m.chat, await(await fetch(hh)).buffer(), `Cᴏɴɢʀᴀᴛᴜʟᴀᴛɪᴏɴ, Aɴᴅᴀ Tᴇʟᴀʜ Nᴀɪᴋ Lᴇᴠᴇʟ!!`, botdate, 'Menu', '.menu', 'Owner', '.owner', 'Speed', '.ping', m, { contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: 'sig',
    mediaType: 2, 
    description: sig,
    title: "Follow Sɪɴɪ Cᴜʏ",
